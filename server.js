...
// Cache token in memory (valid 2 hours)
let _ebayToken = null;
...
async function getEbayToken() {
  ...
  if (!clientId || !clientSecret) {
    throw new Error("EBAY_CLIENT_ID و EBAY_CLIENT_SECRET غير مضبوطين في .env");
  }
...
// GET /api/ebay/search?q=...&limit=...&category=...
// البحث عن منتجات eBay
app.get("/api/ebay/search", async (req, res) => {
  const { q = "electronics", limit = 12, category, minPrice, maxPrice, sort } = req.query;
  const isSandbox = process.env.EBAY_ENV !== "production";
  ...
  try {
    ...
    const params = new URLSearchParams({
      q:     q.toString().slice(0, 100),
      limit: Math.min(parseInt(limit) || 12, 40).toString(),
    });
    if (category)  params.append("category_ids", category);
    if (minPrice)  params.append("filter", `price:[${minPrice}..${maxPrice || "*"}],priceCurrency:USD`);
    if (sort)      params.append("sort", sort); // PRICE, -PRICE, NEWLY_LISTED, ENDING_SOONEST

    const r = await fetch(`${baseUrl}/item_summary/search?${params}`, {
      headers: {
    ...
    if (!r.ok) {
      ...
      return res.json({ ok: false, msg: "eBay API error: " + (err.errors?.[0]?.message || r.status) });
    }

    const data = await r.json();

    // تحويل نتائج eBay لصيغة NovaShop
    const products = (data.itemSummaries || []).map(item => ({
      ...
      category:     item.categories?.[0]?.categoryName || "عام",
      rating:       item.seller?.feedbackScore ? Math.min(5, item.seller.feedbackScore / 1000) : 4.0,
      reviews:      item.seller?.feedbackPercentage ? Math.round(item.seller.feedbackPercentage) : 0,
      ebayUrl:      item.itemWebUrl,
      condition:    item.condition || "Used",
      location:     item.itemLocation?.country || "",
      shipping:     item.shippingOptions?.[0]?.shippingCost?.value || "0",
      source:       "ebay",
      badge:        item.topRatedBuyingExperience ? "top" : (item.condition === "NEW" ? "new" : null),
    }));
...
// GET /api/ebay/item/:id — تفاصيل منتج محدد
app.get("/api/ebay/item/:id", async (req, res) => {
  ...
  try {
    const token = await getEbayToken();
    const itemId = req.params.id.replace("ebay_", "");

    const r = await fetch(`${baseUrl}/item/${itemId}`, {
      headers: {
        "Authorization":           `Bearer ${token}`,
        "X-EBAY-C-MARKETPLACE-ID": "EBAY_US",
      },
    ...
    if (!r.ok) return res.json({ ok: false, msg: "المنتج غير موجود" });
    ...
    res.json({
      ...
      product: {
        ...
        currency:    item.price?.currency || "USD",
        image:       item.image?.imageUrl || "",
        images:      item.additionalImages?.map(i => i.imageUrl) || [],
        category:    item.categories?.[0]?.categoryName || "عام",
...
// GET /api/ebay/categories — فئات eBay الشائعة
app.get("/api/ebay/categories", async (req, res) => {
  // فئات ثابتة من eBay (الأكثر شيوعاً)
  res.json({
    ok: true,
    categories: [
      { id: "9355",   name: "موبايلات وهواتف",  icon: "📱" },
      { id: "58058",  name: "لابتوب وكمبيوتر",   icon: "💻" },
      { id: "15032",  name: "كاميرات وتصوير",    icon: "📷" },
      { id: "1249",   name: "فيديو جيم",         icon: "🎮" },
      { id: "11450",  name: "ملابس وأزياء",       icon: "👗" },
      { id: "26395",  name: "صحة وجمال",         icon: "💄" },
      { id: "11700",  name: "بيت وحديقة",        icon: "🏠" },
      { id: "888",    name: "رياضة ولياقة",       icon: "⚽" },
      { id: "625",    name: "مجوهرات وساعات",     icon: "💍" },
      { id: "220",    name: "ألعاب أطفال",        icon: "🧸" },
    ],
  });
});
