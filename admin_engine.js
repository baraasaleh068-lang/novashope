  ...
  orders.forEach(order => {
    ...
    div.innerHTML = `
      <div class="order-admin-top">
        <div>
          <div class="order-admin-id">${order.id}</div>
      ...
      <div class="order-admin-actions">
        ${order.status==="pending" ? `
          <button class="confirm-btn" onclick="changeStatus('${order.id}','confirmed')">✅ قبول التحويل وتأكيد الطلب</button>
          <button class="reject-btn"  onclick="changeStatus('${order.id}','rejected')">❌ رفض</button>` : ""}
        ${order.status==="confirmed" ? `<button class="ship-btn"    onclick="changeStatus('${order.id}','shipped')">🚚 تم الشحن</button>` : ""}
        ${order.status==="shipped"   ? `<button class="deliver-btn" onclick="changeStatus('${order.id}','delivered')">📦 تم التسليم</button>` : ""}
